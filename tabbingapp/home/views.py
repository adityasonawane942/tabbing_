...
from .models import Tournament, Adjudicator, Team, Institution, Venue, Round, Room
from .forms import UserForm, Upload_Institution_Form, Upload_Adjudicator_Form, Upload_Team_Form, Upload_Venue_Form, Create_Round_Form
...
class RoundCreate(TemplateView):
    template = "home/round_form.html"

    def get(self, request, pk):
        form = Create_Round_Form()
        args = {'form': form}
        return render(request, self.template, args)

    def post(self, request, pk):
        form = Create_Round_Form(request.POST)
        if form.is_valid():
            post = form.save(commit=False)

            the_tournament = Tournament.objects.filter(id=pk).get()
            post.tournament = the_tournament

            no_of_teams = len(Team.objects.filter(tournament=the_tournament))
            post.number_of_teams = no_of_teams

            no_of_adjudicators = len(Adjudicator.objects.filter(tournament=the_tournament))
            post.number_of_adjudicators = no_of_adjudicators

            no_of_rooms = no_of_teams//4
            post.number_of_rooms = no_of_rooms

            no_of_adjudicators_in_a_room = no_of_rooms//no_of_adjudicators
            post.number_of_adjudicators_in_a_room = no_of_adjudicators_in_a_room
            ...
            for i in range(no_of_rooms):
                ...
                r.team1 = Team.objects.filter(tournament=pk).order_by('score')[4*i]
                r.team2 = Team.objects.filter(tournament=pk).order_by('score')[(4*i)+1]
                r.team3 = Team.objects.filter(tournament=pk).order_by('score')[(4*i)+2]
                r.team4 = Team.objects.filter(tournament=pk).order_by('score')[(4*i)+3]

                adjudicators = Adjudicator.objects.filter(tournament=pk).order_by('score')
                for j in range(len(adjudicators)):
                    if (j-i)%no_of_rooms == 0:
                        r.adjudicators = r.adjudicators + adjudicators[j].adjudicator_name

                r.save()

            return redirect('home:rounds', pk)
